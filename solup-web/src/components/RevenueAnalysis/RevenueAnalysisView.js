...
function RevenueAnalysisView() {
  ...
  const radialBarTotal = radialBarData ? radialBarData.reduce((acc, item) => acc + item.deposit, 0) : 0;

  // 전월 매출 데이터
  const previousData = radialBarData?.filter((item) => item.tradeDate.startsWith("2023-08"));
  const previousTotal = previousData?.reduce((acc, item) => acc + item.deposit, 0);

  // 현재 매출 데이터
  const currentData = radialBarData?.filter((item) => item.tradeDate.startsWith("2023-09"));
  const currentTotal = currentData?.reduce((acc, item) => acc + item.deposit, 0);
  ...
  const radialBarOptions = {
    ...
    // 매출 시계열 그래프
const lineData = SalesAnalysisData
    ...
    <div>
      {isLoading ? (
        ...
        <div>
          ...
          <ReactApexChart options={radialBarOptions} series={[ratio.toFixed(0)]} type="radialBar" />
          ...
          <ReactApexChart options={donutChartOptions} series={donutChartSeries} type="donut" />
